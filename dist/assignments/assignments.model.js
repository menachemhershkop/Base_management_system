"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Assignment = void 0;
const sequelize_typescript_1 = require("sequelize-typescript");
const shifts_model_1 = require("../shifts/shifts.model");
const users_model_1 = require("../users/users.model");
let Assignment = class Assignment extends sequelize_typescript_1.Model {
};
exports.Assignment = Assignment;
__decorate([
    (0, sequelize_typescript_1.Column)(sequelize_typescript_1.DataType.NUMBER),
    (0, sequelize_typescript_1.ForeignKey)(() => users_model_1.User),
    __metadata("design:type", Number)
], Assignment.prototype, "userId", void 0);
__decorate([
    (0, sequelize_typescript_1.ForeignKey)(() => shifts_model_1.Shift),
    (0, sequelize_typescript_1.Column)(sequelize_typescript_1.DataType.NUMBER),
    __metadata("design:type", Number)
], Assignment.prototype, "shiftId", void 0);
__decorate([
    (0, sequelize_typescript_1.BelongsTo)(() => users_model_1.User),
    __metadata("design:type", users_model_1.User)
], Assignment.prototype, "user", void 0);
__decorate([
    (0, sequelize_typescript_1.BelongsTo)(() => shifts_model_1.Shift),
    __metadata("design:type", shifts_model_1.Shift)
], Assignment.prototype, "shift", void 0);
exports.Assignment = Assignment = __decorate([
    sequelize_typescript_1.Table
], Assignment);
//# sourceMappingURL=assignments.model.js.map